...
class Player(Entity):
    ...
    def animate(self,dt):
        ...
        if int(self.frame_index) == 2 and self.attacking and not self.bullet_shot:
            bullet_start = self.rect.center + self.bullet_direction * 80
            self.create_bullet(bullet_start,self.bullet_direction)
            self.bullet_shot = True

        # resets the frame index after it's done playing the animation through each cycle
